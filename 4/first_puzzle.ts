const puzzle = `bdwdjjo avricm cjbmj ran lmfsom ivsof
...
const passPhrases = puzzle.split('\n');

function validatePassPhrases(phrase: String): Boolean {
  const sortedPhraseArray = phrase.split(' ').sort();

  for (let i = 0; i < sortedPhraseArray.length; i++) {
    if (sortedPhraseArray[i] === sortedPhraseArray[i + 1]) {
      return false;
    }
  }
  return true;
...
function validPassPhrases(): Number {
  const validPhrases = passPhrases.filter(phrase =>
    validatePassPhrases(phrase)
  );
